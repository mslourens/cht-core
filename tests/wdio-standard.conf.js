...
const uploadStandardConfig = async () => {
  try {
    console.log(`Executing cht-conf from Standard Config`);
    ...
    const { stdout } = await exec(`./node_modules/.bin/cht --url=http://admin:pass@localhost:${apiPort} --force --no-check`,
      { cwd: 'config/standard' });
    console.log(`Executing cht-conf from Standard Config Completed: ${stdout}`);
    return stdout;
  } catch (err) {
    console.log(`Error encountered while executing cht-conf from Standard Config: ${err.message}`);
    return err.stdout;
  }
};
